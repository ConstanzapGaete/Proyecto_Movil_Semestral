...
export class JustificaraPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.authSubscription = this.firebaseService.getAuthState().subscribe(async (user) => {
      if (user?.email) {
        this.ausencias = await this.basededatosService.obtenerFechasAusentes(user.email);
      }
    });
  }
