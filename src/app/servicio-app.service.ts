...
export class ServicioAppService {
  ...
  autenticarUsuario(usuario: string, password: string): boolean {
    const user = this.usuarios.find(user => user.usuario.trim().toLowerCase() === usuario.trim().toLowerCase() && user.password === password);
    if (user) {
  ...
  verificarUsuarioExistente(usuario: string): boolean {
    return this.usuarios.some(user => user.usuario.trim().toLowerCase() === usuario.trim().toLowerCase());
  }
